const mineflayer = require('mineflayer'); const { pathfinder, Movements, goals } = require('mineflayer-pathfinder'); const { GoalNear, GoalBlock } = goals; const { Vec3 } = require('vec3'); const express = require('express'); const fs = require('fs');
...
if (!hasWood && wood) { bot.chat('أبحث عن خشب وسأبدأ تكسيره!'); await bot.pathfinder.goto(new GoalBlock(wood.position.x, wood.position.y, wood.position.z)); try { await wait(Math.random() * 1000 + 500); await bot.dig(wood); } catch (err) { bot.chat('❌ فشل في كسر الخشب: ' + err.message); } return; }

if (hasWood && !hasCraftingTable) { const craftingTableRecipe = mcData.recipes.craftingTable?.[0]; if (craftingTableRecipe) { bot.chat('سأصنع طاولة التصنيع'); try { await wait(Math.random() * 1000 + 500); await bot.craft(craftingTableRecipe, 1, null); } catch (err) { bot.chat('❌ فشل في صنع الطاولة: ' + err.message); } } return; }

if (hasWood && hasCraftingTable && !hasPickaxe) { bot.chat('أحتاج إلى حجر لصنع فأس حجري'); const stone = bot.findBlock({ matching: block => mcData.blocks[block.type].name === 'stone', maxDistance: 32 }); if (stone) { await wait(Math.random() * 1000 + 500); await bot.pathfinder.goto(new GoalBlock(stone.position.x, stone.position.y, stone.position.z)); } return; }

bot.chat('✅ مستعد للتطوير والمهام!'); await wait(Math.random() * 3000 + 1000); exploreRandomly(); }

async function exploreRandomly() { if (!bot.entity) return; const x = bot.entity.position.x + Math.floor(Math.random() * 20 - 10); const z = bot.entity.position.z + Math.floor(Math.random() * 20 - 10); const y = bot.entity.position.y; bot.look(Math.random() * Math.PI * 2, 0); // نظرة عشوائية await wait(Math.random() * 4000 + 2000); bot.pathfinder.setGoal(new GoalBlock(x, y, z)); }
...
setInterval(() => {
  if (Math.random() < 0.2 && bot.chat) {
    const phrases = ['يبدو أن الجو جميل!', 'هل يوجد أحد هنا؟', 'أين أذهب الآن؟', 'أحتاج إلى الطعام قريبًا.'];
...
bot.on('goal_reached', async () => { console.log('🎯 الهدف تم الوصول إليه! اختيار هدف جديد ...'); await wait(Math.random() * 3000 + 1000); exploreRandomly(); });
