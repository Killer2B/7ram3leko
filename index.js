const mineflayer = require('mineflayer'); const { pathfinder, Movements, goals } = require('mineflayer-pathfinder'); const { GoalNear, GoalBlock } = goals; const autoeat = require('mineflayer-auto-eat').plugin; const armorManager = require('mineflayer-armor-manager'); 
//const { mineflayer: mineflayerViewer } = require('prismarine-viewer'); 
const express = require('express'); const fs = require('fs'); const { Vec3 } = require('vec3');
...
const diaryFile = './diary.json'; const memoryFile = './memory.json'; let bot; let reconnectDelay = 5000; let isConnecting = false; let deathCount = 0; const knownLocations = { villages: [], resources: {} };
...
async function evolveBot() { if (!bot.chat || typeof bot.chat !== 'function') return; const mcData = require('minecraft-data')(bot.version); const inventory = bot.inventory.items().map(i => i.name); const hasWood = inventory.includes('oak_log') || inventory.some(i => i.includes('_log')); const hasCraftingTable = inventory.includes('crafting_table'); const hasPickaxe = inventory.some(i => i.includes('pickaxe'));
...
function createBot() { bot = mineflayer.createBot({ host, username, version });

bot.loadPlugin(pathfinder); bot.loadPlugin(autoeat); bot.loadPlugin(armorManager);

bot.once('spawn', () => { console.log('✅ Bot spawned'); mineflayerViewer(bot, { port: 3001, firstPerson: false }); const mcData = require('minecraft-data')(bot.version); const defaultMove = new Movements(bot, mcData); bot.pathfinder.setMovements(defaultMove);
...
bot.on('chat', (username, message) => { if (username === bot.username) return; if (message === 'تعال') { const player = bot.players[username]; if (!player || !player.entity) { bot.chat('لا أستطيع رؤيتك'); return; } const goal = new GoalNear(player.entity.position.x, player.entity.position.y, player.entity.position.z, 1); bot.pathfinder.setMovements(new Movements(bot)); bot.pathfinder.setGoal(goal); bot.chat('أنا قادم إليك'); } });
...
function checkServerAndStart() { if (isConnecting) return; isConnecting = true; console.log('⏳ Checking server and starting bot...'); createBot(); }
